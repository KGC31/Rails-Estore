class User < ApplicationRecord
  ...
  validates :email, uniqueness: { case_sensitive: false, message: "Email has already been taken" }

  # Validate password (not password_digest directly, assuming you're using bcrypt for password hashing)
  validates :password, format: { with: /\A(?=.*[0-9])(?=.*[!@#$%^&*])[a-zA-Z0-9!@#$%^&*]{6,}\z/, message: "must be at least 6 characters, and include one number and one letter." }
